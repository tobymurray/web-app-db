...
class DbConfig {
  constructor(config) {
    ...
    this.superuserDefaultDatabase = new DbUser(config.defaultDatabase, config.superuserName, config.superuserPassword);
    this.superuserTestDatabase = new DbUser(this.testDatabaseName, config.superuserName, config.superuserPassword);
  ...
  revokeFunctionsFromPublic(client) {
    return client.query(this._alterDefaultPrivileges() + " REVOKE EXECUTE ON FUNCTIONS FROM PUBLIC");
  }
  ...
  enableCiTextExtension(client) {
    return client.query("CREATE EXTENSION citext WITH SCHEMA \"" + this.databaseSchemaName + "\"");
  }

  setAdminDefaultSchema(client) {
    return client.query("ALTER ROLE \"" + this.adminName + "\" SET search_path TO \"" + this.databaseSchemaName + "\"");
  }

  setUserDefaultSchema(client) {
    return client.query("ALTER ROLE \"" + this.userName + "\" SET search_path TO \"" + this.databaseSchemaName + "\"");
  }

  _alterDefaultPrivileges() {
    return "ALTER DEFAULT PRIVILEGES FOR ROLE \"" + this.adminName + "\"";
  }

  _alterDefaultPrivilegesInSchema() {
    return this._alterDefaultPrivileges() + " IN SCHEMA \"" + this.databaseSchemaName + "\"";
  }
